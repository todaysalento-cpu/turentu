...
export default function LivePage() {
  ...
  const [pending, setPending] = useState<Corsa[]>([]);
  const [corseGiornata, setCorseGiornata] = useState<Corsa[]>([]);
  const [loadingId, setLoadingId] = useState<number | null>(null);
  const [disponibilita, setDisponibilita] = useState<any[]>([]);
  const [corsaSelezionata, setCorsaSelezionata] = useState<Corsa | null>(null);
  const [unreadCounts, setUnreadCounts] = useState<Record<number, number>>({});
  const [chatMessages, setChatMessages] = useState<Record<number, Messaggio[]>>({});
  ...
  useEffect(() => {
    ...
    socket.on('new_message', (message: Messaggio) => {
  ...
  const handleAccetta = async (id: number) => {
    ...
    try {
      ...
      toast.success('Corsa accettata!');
    } catch (err: any) { toast.error(err.message); }
    finally { setLoadingId(null); }
  };

  const handleToggleCorsa = async (corsa: Corsa) => {
    if (loadingId) return;
    ...
    try {
      ...
      if (!res.ok) { toast.error(`Errore ${endpoint} corsa`); return; }
      ...
      if (endpoint === 'end') {
        if (corsaAggiornata.pagamentiCatturati?.length) {
          corsaAggiornata.pagamentiCatturati.forEach(p =>
            toast.success(`Prenotazione #${p.prenotazione_id}: €${p.importo.toFixed(2)} → ${p.stato}`)
          );
        } else {
          toast.success('Corsa completata e pagamenti catturati.');
        }
      }
    ...
    <div className="w-full max-w-[720px] mx-auto bg-white space-y-8 py-6">
      ...
      <section className="space-y-4">
        ...
        <CorseGiornata
          ...
          loadingId={loadingId}
          onSelectCorsa={setCorsaSelezionata}
        />
      </section>
