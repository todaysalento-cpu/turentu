...
export default function BottomSheetCalendario({ value, onClose, onConfirm }: BottomSheetCalendarioProps) {
  ...
  const handleConfirm = () => {
    ...
    onConfirm(finalDate.toISOString().slice(0, 16));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-end">
      ...
      {/* Sheet */}
      <div className="relative w-full max-w-md bg-white rounded-t-xl p-4 shadow-lg border border-gray-200">

        {/* Header */}
        <div className="flex justify-between items-center mb-3">
          {/* Freccia indietro compatta */}
          <button
            onClick={step === "hour" ? () => setStep("day") : onClose}
            className="text-[#ff3036] font-bold text-lg w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#ffe5e5]"
          >
            ←
          </button>

          <span className="font-medium text-gray-700">{step === "day" ? "Seleziona giorno" : "Seleziona ora"}</span>

          {/* X chiusura */}
          <button
            onClick={onClose}
            className="text-gray-400 font-bold text-lg w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100"
          >
            ×
          </button>
        </div>
            ...
            {["oggi", "domani", "altro"].map((t) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium
                  ${tab === t ? "bg-[#ff3036] text-white shadow-md" : "bg-gray-100 text-gray-700"}`}
              >
              ...
              {monthLabels.map((m, idx) => (
                <button
                  ...
                  className={`flex-shrink-0 px-3 py-1 rounded-full text-sm font-medium
                    ${idx === currentMonthIndex ? "bg-[#ff3036] text-white shadow-md" : "bg-gray-100 text-gray-700"}`}
                >
                  {m}
                </button>
              ))}
              ...
              {days
                ...
                .map((d, idx) => (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center min-w-[60px] p-2 rounded-lg border-2
                      ${d.toDateString() === selectedDay.toDateString() ? "border-[#ff3036] bg-[#ffe5e5]" : "border-gray-200 bg-gray-50"}
          ...
          <div className="flex gap-2 overflow-x-auto py-2 scroll-snap-x">
            {HOURS.map((h) => (
              <button
                ...
                className={`min-w-[60px] py-2 px-3 rounded-lg border-2 flex-shrink-0
                  ${h === selectedHour ? "border-[#ff3036] bg-[#ffe5e5]" : "border-gray-200 bg-gray-50"}
