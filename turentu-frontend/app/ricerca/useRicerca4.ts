...
function formatDurata(minuti: number | undefined) {
...
export function useRicerca() {
  ...
  const risultatiFiltrati = useMemo(() => {
    const filtered = filtroSlot === "Tutte"
      ? risultati
      : risultati.filter(r => r.stato?.includes(filtroSlot === "Liberi" ? "libero" : "prenotabile"));

    console.log("🔹 Risultati filtrati:", filtered.map(r => ({ id: r.id, stato: r.stato })));
    return filtered;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("📤 Richiesta inviata:", body);
      ...
      console.log("✅ Risultati backend:", data);

      // --- Aggiorna durataMinuti e formatta
      data = Array.isArray(data) ? data.map((r: Risultato) => ({
        ...r,
        durataMinuti: r.durataMinuti ?? convertIntervalToMinutes(r.durata),
        durataFormattata: formatDurata(r.durataMinuti ?? convertIntervalToMinutes(r.durata))
      })) : [];
  ...
  /* Converte intervallo PostgreSQL "hh:mm:ss" → minuti */
  function convertIntervalToMinutes(interval: string | undefined) {
  ...
  return {
    ...
    isMobile,
    user,
    formatDurata // esporta helper per visualizzazione in component
  };
}