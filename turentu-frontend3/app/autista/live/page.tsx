...
// Tipo preciso per ogni corsa
interface Corsa {
  id: number;
  stato?: string;
  origine_address?: string;
  destinazione_address?: string;
...
// Props aggiornate con unreadCounts opzionale
interface CorseGiornataProps {
  corse?: Corsa[];
  onToggleCorsa?: (corsa: Corsa) => void;
  loadingId?: number | null;
  onSelectCorsa?: (corsa: Corsa | null) => void;
  unreadCounts?: Record<number, number>;
}

export default function CorseGiornata({
  corse = [],
  onToggleCorsa,
  loadingId,
  onSelectCorsa,
  ...
  // Filtra solo corsa prenotabile o in_corso
  useEffect(() => {
    setCorseGiornata(
      (corse || []).filter(c => {
                  ...
                  <button
                    onClick={() => {
                      ...
                      const corsaForChat = {
                        ...c,
                        stato: 'in_corso',
                        origine: c.origine || c.origine_address,
                        destinazione: c.destinazione || c.destinazione_address
                      };
                      onSelectCorsa?.(corsaForChat);
                    }}
