...
// Default coerente con le colonne esistenti
...
/**
 * Recupera le tariffe dal DB usando i nomi colonna corretti
 */
...
/**
 * Recupera dettaglio pool con join per recuperare i posti
 */
...
/**
 * Calcolo prezzo finale con gestione errori robusta
 */
export async function calcolaPrezzo(corsa, postiRichiesti, tipo, kmUtente, kmTotali, totPasseggeriCorrenti = 0, classe = 'STANDARD') {
  ...
  console.log(`💰 [PRICING] Tipo: ${tipoValido} | Classe: ${classe} | KM: ${kmUtente.toFixed(2)}`);
  ...
  try {
    switch (tipoValido) {
      ...
      case 'pop-bus':
        ...
        if (poolData.length === 0) return PREZZO_MINIMO;
