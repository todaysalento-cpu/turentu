...
export async function prenotaCorsa(corsa, clienteId, postiRichiesti, client) {
  ...
  try {
    ...
    const { posti_disponibili, posti_prenotati = 0, posti_totali, primo_posto } = res.rows[0];
    if (posti_disponibili < postiRichiesti) throw new Error('Posti insufficienti');

    // Aggiorna posti disponibili, prenotati e primo_posto se è la prima prenotazione
    await client.query(
      `UPDATE corse
