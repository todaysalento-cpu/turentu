...
describe('Test Suite: Matching e Attivazione con Disponibilità Veicolo Reale', () => {
  ...
  test('Dovrebbe interrogare la disponibilità reale dei veicoli e tariffe dal DB per determinare l attivazione', async () => {
    ...
    const { rows: nodi } = await client.query(`
      ...
      RETURNING id, nome_nodo, posizione
    `);
    ...
    // 🔍 CALCOLO ESPLICITO DELLA DISTANZA DA DB PER VISUALIZZARLA NEL LOG DEL TEST
    const { rows: calcDistanza } = await client.query(`
      SELECT ST_Distance(
        (SELECT posizione FROM nodi_direttrice WHERE id = $1)::geography,
        (SELECT posizione FROM nodi_direttrice WHERE id = $2)::geography
      ) / 1000 AS distanza_km
    `, [nodeA, nodeB]);
    
    const distanzaKmReale = parseFloat(calcDistanza[0].distanza_km);
    const postiTotali = 25;
    const ricavoStimatoPrevisto = postiTotali * 2.50; // Come da logica: posti_occupati * 2.50
    const sogliaAttivazioneMinima = distanzaKmReale * euroKmReale;

    console.log('\n--- [TEST METRICS / CALCOLO SOGLIA] ---');
    console.log(`📍 Distanza calcolata tra i nodi: ${distanzaKmReale.toFixed(3)} km`);
    console.log(`💰 Tariffa applicata: ${euroKmReale} €/km`);
    console.log(`🎯 Soglia di attivazione minima stimata: ${sogliaAttivazioneMinima.toFixed(2)} € (Distanza * Tariffa)`);
    console.log(`💵 Ricavo attuale stimato (${postiTotali} posti * 2.50€): ${ricavoStimatoPrevisto.toFixed(2)} €`);
    console.log(`⚖️ Verifica sostenibilità: ${ricavoStimatoPrevisto >= sogliaAttivazioneMinima ? 'SUPERATA (Il segmento si attiva)' : 'NON SUPERATA'}`);
    ...
    console.log(`   - Ricavo Stimato sul DB: ${segmenti[0].ricavo_stimato} €`);
