...
async function testPrezzoSlotECorse() {
  ...
  try {
    ...
    risultati.forEach(slot => {
      if (slot.slot_libero) {
        // distanza per il prezzo: preferisci quella calcolata dal servizio
        const distanzaPrezzo = slot.distanzaPrezzoKm ?? haversineDistance(
          richiesta.coord,
          richiesta.coordDest
        );

        const euroKm = Number(slot.euro_km);
        const prezzoAtteso = euroKm
          ? distanzaPrezzo * euroKm * richiesta.posti_richiesti
          : null;

        console.log(`Slot libero: Veicolo ${slot.veicolo_id}`);
        console.log(`  Prezzo calcolato: ${slot.prezzo.toFixed(2)} €`);
        console.log(`  Euro/km: ${euroKm ? euroKm.toFixed(2) : 'N/D'}`);
        ...
        if (prezzoAtteso !== null) {
          console.log(
            `  Verifica prezzo: ${distanzaPrezzo.toFixed(2)} × ${euroKm.toFixed(2)} = ${prezzoAtteso.toFixed(2)} €`
          );
        }
      } else {
