...
router.post('/:id/accetta', async (req, res) => {
  ...
  try {
    ...
    // 🚫 Se non è più pending → blocco
    if (selectedPending.stato !== 'pending') {
      await client.query('ROLLBACK');
      return res.status(400).json({ 
        message: 'Questa richiesta non è più disponibile' 
      });
    }
    ...
    if (selectedPending.request_id) {
      ...
      if (alreadyAccepted.rows.length) {
        await client.query('ROLLBACK');
        return res.status(400).json({ 
          message: 'Richiesta già accettata da un altro autista' 
        });
      }
    }
    ...
    for (const p of acceptedPendings) {
      ...
      // 🔎 Controllo corsa esistente prima di crearne una nuova
      if (!p.corsa_id) {
        ...
        if (existingCorsa.rows.length) {
          ...
          // crea solo prenotazione su corsa esistente
