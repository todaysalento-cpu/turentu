...
const formReducer = (state: RicercaForm, action: FormAction): RicercaForm => {
  switch (action.type) {
    ...
    case "SET_DATETIME":
      return { ...state, start_datetime: toUTCISOString(action.payload) };
    case "SET_POSTI":
      return { ...state, posti_richiesti: action.payload };
    default:
      return state;
  }
};
...
export function useRicerca() {
  ...
  const risultatiFiltrati = useMemo(() => {
    ...
    const statoMap: Record<FiltroSlot, string> = {
      Tutte: "",
      Liberi: "libero",
      Prenotabili: "prenotabile",
    };

    return risultati.filter(r => r.stato === statoMap[filtroSlot]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 🔹 Mantieni lo stato come viene dal backend, senza calcoli
      data = Array.isArray(data)
        ? data.map((r: Risultato) => ({
            ...r,
            durataFormattata: formatDurata(r.durataMinuti),
            start_datetime_backend: r.start_datetime,
            stato: r.stato, // <-- qui lo stato viene dal backend
          }))
        : [];

      console.log(
        "🔹 Risultati mappati:",
        data.map(r => ({ id: r.id, stato: r.stato }))
      );
