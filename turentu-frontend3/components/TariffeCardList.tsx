...
import { Tariffa, TipoTariffa } from './TariffeForm';
...
// Colori per tipo tariffa
const tipoColors: Record<TipoTariffa, string> = {
  standard: 'bg-blue-200 text-blue-800',
  notturna: 'bg-purple-200 text-purple-800',
  festivo: 'bg-orange-200 text-orange-800',
};

// Ordine fisso per visualizzazione
const tipiOrdine: TipoTariffa[] = ['standard', 'notturna', 'festivo'];
...
export default function TariffeCardList({
  ...
  // Ordina le tariffe secondo l'ordine fisso
  const tariffeOrdinate = tipiOrdine
    .map(tipo => tariffe.find(t => t.tipo === tipo))
    .filter((t): t is Tariffa => t !== undefined);
        ...
        {hasTariffa ? (
          <div className="mt-1 text-[9px] flex flex-col gap-1">
            {hasSimulazione ? (
              ...
              <div className="flex items-center gap-1">
                <span className="italic text-gray-700 text-[9px]">
                  Inserisci corsa per stimare guadagno
                </span>
                {onSimula && (
                  <button
                    onClick={onSimula}
          ...
          {tariffeOrdinate.map(t => {
            ...
            return (
              <div key={t.tipo} className="flex gap-1 items-center">
                <div
                  className={`px-1.5 py-0.5 rounded text-[9px] font-medium ${tipoColors[t.tipo]} text-center min-w-[60px]`}
                >
                  {t.tipo}
                </div>
                ...
                <div
                  className={`px-1 py-0.5 rounded text-[9px] text-center min-w-[70px] flex-shrink-0 ${highlightColor}`}
                >
