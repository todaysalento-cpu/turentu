...
export default function GenericTable<T extends Veicolo>({
  ...
  const sortedData = data
    .map(row => {
      ...
      const end = row.fine;
      const inattivita = Array.isArray(row.inattivita) ? row.inattivita : [];
      const giorniEsclusi = Array.isArray(row.giorni_esclusi) ? row.giorni_esclusi : [];
                ...
                <div className="text-[0.55rem] text-gray-800 mt-1 space-y-1">
                  ...
                  <div className="flex gap-2">
                    ...
                    <span className="flex-1 flex flex-col">
                      {Array.isArray(inattivita) && inattivita.length
      ...
      {popupVeicolo && (
        <Modal onClose={() => setPopupVeicolo(null)}>
          <DisponibilitaForm
            ...
            inattivita={Array.isArray(popupVeicolo.inattivita) ? popupVeicolo.inattivita : []}
            newInizioInatt=""
            newFineInatt=""
            ...
            onAddInattivita={(start, fine) =>
              setPopupVeicolo(prev =>
                prev
                  ? { ...prev, inattivita: [...(Array.isArray(prev.inattivita) ? prev.inattivita : []), { start, fine }] }
                  : prev
              )
            }
            onRemoveInattivita={idx =>
              setPopupVeicolo(prev =>
                prev
                  ? { ...prev, inattivita: (Array.isArray(prev.inattivita) ? prev.inattivita : []).filter((_, i) => i !== idx) }
                  : prev
              )
            }
            onChangeNewInizio={() => {}}
