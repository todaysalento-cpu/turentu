  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (resetMode) {
        ...
        // 🔹 Login tramite UserContext
        await login(email, password); // qui viene usato il contesto, non fetch manuale

        // Reindirizza in base al ruolo
        if (onSuccess) onSuccess();
        else {
          const storedUser = JSON.parse(sessionStorage.getItem('user') || '{}');
          if (storedUser.role === 'cliente') router.push('/cliente/prenotazioni');
          else if (storedUser.role === 'autista') router.push('/autista/corse');
          else router.push('/admin/dashboard');
        }
      }
    } catch (err: any) {
      console.error('❌ LoginForm error:', err.message || err);
