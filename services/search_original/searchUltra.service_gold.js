...
const GEOHASH_PRECISION = 7; // precisione geohash per filtraggio rapido
const MAX_DEST_KM = 5;       // tolleranza destinazione per corse compatibili
...
export async function loadCachesUltra() {
  ...
  try {
    ...
    veicoliCache = veicoliRes.rows.reduce((acc, v) => {
      acc[v.id] = { ...v, geohash: ngeohash.encode(v.lat, v.lon, GEOHASH_PRECISION) };
      return acc;
    }, {});
...
// =========================
// Calcolo disponibilità grezza rapida
// =========================
export function getDisponibilitaUltra(richiesta) {
  ...
  const corse = corseCache.filter(c => {
    ...
    // distanza origine (geohash + distanza reale)
    const distanzaOrigine = richiesta.coord
      ? haversineDistance(richiesta.coord, { lat: c.origine_lat, lon: c.origine_lon })
      : 0;
    if (richiesta.coord && distanzaOrigine > params.tolleranzaKm) return false;

    // distanza destinazione (solo distanza reale)
    if (richiesta.destinazione) {
      const distanzaDest = haversineDistance(richiesta.destinazione, { lat: c.dest_lat, lon: c.dest_lon });
      if (distanzaDest > MAX_DEST_KM) return false;
    }
...
export async function cercaSlotUltra(richiesta) {
  ...
  const corsaResults = await Promise.all(topCorse.map(async corsa => {
    ...
    const prezzo = await calcolaPrezzo(
      { km: richiesta.km, tipo_corsa: corsa.tipo_corsa, posti_prenotati: corsa.posti_prenotati, primo_posto: corsa.primo_posto },
      corsa.euro_km ?? 1.0,
      1.0,
      richiesta.posti_richiesti,
      statoPrezzo
