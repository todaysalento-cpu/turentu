...
export default function StepPagamento({
  ...
  useEffect(() => {
    ...
    // Collegamento socket solo se almeno uno slot ha pending_id
  ...
  const handlePayment = async () => {
    ...
    try {
      // 🔹 Prepara payload con fallback sicuri
      const safeSlots = slots.map(slot => ({
        ...
        origine: slot.origine ?? { lat: 45.4642, lon: 9.1900 },
        destinazione: slot.destinazione ?? { lat: 45.4650, lon: 9.1850 },
        localitaOrigine: slot.localitaOrigine ?? "Piazza Duomo, Milano",
        localitaDestinazione: slot.localitaDestinazione ?? "Corso Vittorio Emanuele II, Milano",
      ...
      // 🔹 Aggiorna pending_id negli slot prima di collegare i socket
