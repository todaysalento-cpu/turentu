...
async function getLocalitaSafeCached(locInput) {
    // Gestione flessibile: stringa, oggetto o coordinate
...
export async function formatResults(richiesta, risultatiFiltrati, corseOriginali) {
    ...
    return (await Promise.all(risultatiDaFormattare.map(async (item) => {
        try {
            ...
            // A. Caso Pool
            if (item.is_pool) {
                const p = await calcolaPrezzo(item, richiesta.posti_richiesti, 'pop-bus', distKm).catch(() => 0);
                const prezzoVal = Number(p) || 0;
                return { ...item, localitaOrigine, localitaDestinazione, prezzo: prezzoVal, prezzo_display: prezzoVal.toFixed(0) };
            }

            // B. Caso Slot Privato
            if (item.tipo === 'privata_slot') {
                const p = await calcolaPrezzo(item, richiesta.posti_richiesti, 'privata', distKm).catch(() => distKm * 0.5);
                const prezzoVal = Number(p) || 0;
                return {
            ...
            const p = await calcolaPrezzo(item, richiesta.posti_richiesti, item.tipo_corsa, distItemKm).catch(() => distItemKm * 0.45);
            const prezzoVal = Number(p) || 0;
