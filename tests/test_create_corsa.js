...
export async function createCorsaFromPending(pending, veicolo, client) {
  ...
  try {
    ...
    // TIPO CORSA
    // ========================
    // fallback: se pending.tipo_corsa è mancante o non valido, impostiamo 'privata'
    let tipoCorsa = pending.tipo_corsa;
    if (!['privata','condivisa'].includes(tipoCorsa)) {
      tipoCorsa = 'privata';
      console.log('⚠️ Tipo corsa non valido, impostato a "privata"');
    }
