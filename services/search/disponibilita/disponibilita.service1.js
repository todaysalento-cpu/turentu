...
export async function getDisponibilita(driver_id) {
  ...
  return res.rows.map(d => {
    ...
    return {
      ...d,
      disponibile, // proprietà per il semaforo verde/rosso
    };
  });
}
...
export async function createDisponibilita(turno) {
  let { veicolo_id, start, fine, manual, giorni_esclusi = [], inattivita = [] } = turno;
  ...
  const res = await pool.query(
    `INSERT INTO disponibilita_veicolo
     ...
     RETURNING *`,
    [veicolo_id, start, fine, manual || false, giorni_esclusi, JSON.stringify(inattivita)]
  );
...
export async function updateDisponibilita(id, update) {
  ...
  if (update.start) update.start = new Date(update.start).toISOString();
  if (update.fine)  update.fine  = new Date(update.fine).toISOString();

  // giorni_esclusi è integer[]
  if (update.giorni_esclusi) {
    update.giorni_esclusi = update.giorni_esclusi.map(Number);
  }

  // inattivita rimane jsonb
