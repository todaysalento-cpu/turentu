...
export default function StepPagamento({
  ...
  const handlePayment = async () => {
    ...
    try {
      const safeSlots = slots.map(({ pending_id, posti_richiesti, durata, start_datetime, origine, destinazione, posti_totali, ...rest }) => {
        if (!start_datetime) throw new Error("start_datetime mancante");
        if (!origine || !destinazione) throw new Error("Origine o destinazione mancanti");

        const tipo_corsa = posti_richiesti === posti_totali ? "privata" : "condivisa";
      ...
      // Aggiorna pending_id negli slot
