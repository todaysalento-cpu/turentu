...
function generatePendingMessage({ role, startAddress, endAddress, startDatetime }) {
  ...
  return `Nuova Richiesta - ${startAddress} → ${endAddress} - ${timeStr}`;
}
  ...
  try {
    ...
    for (const slot of slots) {
      ...
      if (is_pool) {
        ...
        // Notifiche integrate tramite Service
        ...
        if (driverId) {
          const message = generatePendingMessage({ role: 'autista', startAddress: localitaOrigine, endAddress: localitaDestinazione, startDatetime: start_datetime });
          
          // Chiamata centralizzata: salva nel DB, invia Socket, invia Push
          await notifyUser(driverId, { 
            type: 'pending', 
            message, 
            role: 'autista',
            data: { corsaId: pItem.id } 
          });
        }
      }
    }
