...
const veicoliRes = await client.query(`
  SELECT id, modello, posti_totali, euro_km, prezzo_passeggero, servizi, ST_Y(coord::geometry) AS lat, ST_X(coord::geometry) AS lon 
  FROM veicolo
`);

veicoliCache = veicoliRes.rows.reduce((acc, v) => { acc[v.id] = { ...v, geohash: ngeohash.encode(v.lat, v.lon, GEOHASH_PRECISION), servizi: v.servizi ? v.servizi.split(',') : [], // <- qui convertiamo in array prezzo_passeggero: Number(v.prezzo_passeggero ?? 1) }; return acc; }, {});
