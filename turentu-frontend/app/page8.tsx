...
import BottomSheetInput from "../components/BottomSheet";
      ...
      <form className="w-full max-w-[1280px] mx-auto p-0  flex flex-col md:flex-row gap-2 items-stretch">
        ...
        <div className="relative flex-1 min-w-0 h-10">
          ...
          <input
            ...
            className="w-full h-10 pl-8 rounded border border-gray-300 focus:border-[#ff3036]"
          />
        </div>
...
{origineOpen && (
  <BottomSheetInput
    value={form.localitaOrigine}
    onChange={(val) => dispatch({ type: "SET_ORIGINE", payload: val })}
    onClose={() => setOrigineOpen(false)}
    placeholder="Da dove parti?"
  />
)}
...
{destinazioneOpen && (
  <BottomSheetInput
    value={form.localitaDestinazione}
    onChange={(val) => dispatch({ type: "SET_DESTINAZIONE", payload: val })}
    onClose={() => setDestinazioneOpen(false)}
    placeholder="Dove vai?"
  />
)}
        ...
        <div className="relative flex-1 min-w-0 h-10">
          ...
          {/* Mobile */}
          <input
            className="w-full h-10 pl-8 rounded border border-gray-300 cursor-pointer md:hidden"
            readOnly
          ...
          {/* Desktop */}
          <input
            className="w-full h-10 pl-8 rounded border border-gray-300 hidden md:block cursor-pointer"
            readOnly
            value={formatReadableDate(form.start_datetime)}
        ...
        {/* Posti */}
        <div className="flex items-center h-10 bg-gray-100 rounded px-2 gap-2 border border-gray-300 flex-none">
          <button
            type="button"
        ...
        {/* Submit */}
        <div className="flex-none md:flex md:items-end">
          <button
            type="submit"
            onClick={handleSubmit}
            disabled={loading}
            className="w-full md:w-auto h-10 bg-[#ff3036] text-white rounded px-4 flex items-center justify-center gap-2"
          >
