...
export default function ChatPanelPopup() {
  ...
  const activeMessages = activeThreadId ? threads[activeThreadId]?.messages || [] : [];

  const incrementUnreadCount = useCallback((key: string) => {
    updateUnreadCount(key, prev => (prev ?? 0) + 1);
  }, [updateUnreadCount]);

  const threadsByCorsa = Object.values(threads).reduce((acc: any, thread) => {
    if (!acc[thread.corsa_id]) acc[thread.corsa_id] = [];
    acc[thread.corsa_id].push(thread);
    return acc;
  }, {});
        ...
        threadsData.forEach((thread: any) => {
          const unreadCount =
            ...
            (thread.messages?.filter((m: Message) =>
              m.sender_id !== user.id && !m.read_status[user.role.toLowerCase()]
            ).length ?? 0);
          ...
          openThread({
            ...
            start_datetime: thread.start_datetime,
            participants: thread.participants || [],
            messages: thread.messages || [],
            unreadCount,
            lastMessageTime: thread.messages?.length
  ...
  }, [socket, user, threads, activeThreadId]);
  ...
  const sendMessage = () => {
    ...
    const msg: Message = {
      ...
      role: user.role.toLowerCase() as 'cliente' | 'autista',
      text: text.trim(),
        ...
        <div className="w-2/5 bg-gray-50 border-r border-gray-200 flex flex-col">
          <div className="p-2 border-b border-gray-200 flex flex-col gap-1">
            {!selectedCorsaId ? (
              ...
              isAutista && threadsByCorsa[selectedCorsaId]?.[0] && (
...
{/* BOX CORSA SELEZIONATA UGUALE CLIENTE */}
<div
  className="flex items-center gap-2 p-2 bg-gray-50 cursor-pointer"
>
  {/* Avatar */}
  ...
  {/* Testo Origine→Destinazione + data */}
  <div className="flex-1 flex flex-col justify-center leading-tight">
    <span className="text-sm font-medium truncate">
      {threadsByCorsa[selectedCorsaId][0].origine.split(',')[0]} → {threadsByCorsa[selectedCorsaId][0].destinazione.split(',')[0]}
    </span>
    ...
    <span className="bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[8px] flex-shrink-0">
      {threadsByCorsa[selectedCorsaId][0].unreadCount}
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {!selectedCorsaId && Object.keys(threadsByCorsa)
              .filter(corsaId => threadsByCorsa[corsaId][0].origine.toLowerCase().includes(searchText.toLowerCase()) ||
                                  threadsByCorsa[corsaId][0].destinazione.toLowerCase().includes(searchText.toLowerCase()))
              .map(corsaId => {
                const threadGroup = threadsByCorsa[corsaId];
                const totalUnread = threadGroup.reduce((sum, t) => sum + (t.unreadCount || 0), 0);
                const thread = threadGroup[0];
                return (
                  <div
                    key={corsaId}
                    onClick={() => isAutista ? setSelectedCorsaId(Number(corsaId)) : setActiveThread(`${thread.corsa_id}_${thread.cliente_id}`)}
                    className="p-2 hover:bg-gray-50 rounded flex justify-between items-center cursor-pointer transition-colors"
                  >
                    <div>
                      <div className="text-xs font-medium">{thread.origine} → {thread.destinazione}</div>
                      <div className="text-[10px] text-gray-500">{formatDate(thread.start_datetime)}</div>
                    </div>
                    {totalUnread > 0 && (
                      <span className="bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[9px]">{totalUnread}</span>
                    )}
                  </div>
                );
              })
            ...
            {isAutista && selectedCorsaId && threadsByCorsa[selectedCorsaId]?.map(thread => {
              ...
              const avatarText = participantName.split(' ').map(n => n[0]).join('').slice(0,2).toUpperCase();
              const colors = ['#f87171', '#fbbf24', '#34d399', '#60a5fa', '#a78bfa', '#f472b6', '#facc15'];
              const avatarColor = colors[participantName.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0) % colors.length];

              return (
                <div
                  key={key}
                  onClick={() => setActiveThread(key)}
                  className={`p-2 rounded flex items-center gap-2 ${activeThreadId === key ? 'bg-gray-100' : 'hover:bg-gray-50'} transition-colors cursor-pointer`}
                >
                  <div
                  ...
                  {thread.unreadCount > 0 && (
                    <span className="bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[8px] flex-shrink-0">{thread.unreadCount}</span>
                  )}
                </div>
              );
            })}
            ...
            <>
              ...
              <div className="flex items-center gap-2 p-2 border-b border-gray-200 bg-gray-50">
                <div
                  ...
                  style={{
                    backgroundColor: (() => {
                      const participant = threads[activeThreadId].participants.find(p => p.id === threads[activeThreadId].cliente_id)
                ...
                <span className="text-sm font-medium truncate">
                  {threads[activeThreadId].participants.find(p => p.id === threads[activeThreadId].cliente_id)?.name
                    || `Cliente ${threads[activeThreadId].cliente_id}`}
                </span>
              ...
              <div className="flex-1 overflow-y-auto px-3 py-2 bg-gray-50">
                <ChatCorsa messages={activeMessages} currentUserRole={user.role.toLowerCase() as 'cliente' | 'autista'} />
              ...
              <div className="px-3 py-2 border-t border-gray-200 flex gap-2 bg-gray-50">
                ...
                />
                <button onClick={sendMessage} className="bg-green-500 text-white px-3 py-1 rounded-full">Invia</button>
              </div>
            </>
          ) : (
