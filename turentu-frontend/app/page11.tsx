...
interface Risultato {
  ...
  prezzo?: number;
  modello: string;
  servizi?: string[]; // contiene "libero" o "prenotabile"
}
...
function Tabs({ filtri, selected, onSelect }: TabsProps) {
  return (
    <div className="flex gap-2 overflow-x-auto py-2 px-2 border-b border-gray-200">
      {filtri.map(f => (
        <button
          key={f}
          onClick={() => onSelect(f)}
...
const filtraRisultatiSlot = (risultati: Risultato[], filtro: FiltroSlot) => {
  ...
  return risultati.filter(r => {
    ...
    if (filtro === "Liberi") return r.servizi.includes("libero");
    if (filtro === "Prenotabili") return r.servizi.includes("prenotabile");
    return true;
  });
};
    ...
    <div className="w-full min-h-screen p-4 flex flex-col gap-4">
      ...
      {isMobile ? (
        mobileView === "form" ? (
          ...
          <div className="flex flex-col gap-3 h-[calc(100vh-20px)]">
            <button
              onClick={() => setMobileView("form")}
              className="px-4 py-2 bg-gray-200 rounded text-center sticky top-0 z-10"
            >
              Modifica ricerca
            </button>
