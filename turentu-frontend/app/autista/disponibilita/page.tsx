...
export default function DisponibilitaPage() {
    ...
    const fetchDisponibilita = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // assicurati che inattivita sia sempre array
          const sanitized = data.map((d: any) => ({
            ...d,
            giorni_esclusi: d.giorni_esclusi || [],
            inattivita: safeArray(d.inattivita),
          }));
          setDisponibilita(sanitized);
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const inattivitaISO = safeArray(inattivita).map(i => ({
    ...
    try {
      ...
      setDisponibilita(prev => {
        ...
        if (idx >= 0) {
          ...
          copy[idx] = {
            ...nuovoTurno,
            giorni_esclusi: nuovoTurno.giorni_esclusi || [],
            inattivita: safeArray(nuovoTurno.inattivita),
          };
          return copy;
        }
        return [...prev, { ...nuovoTurno, giorni_esclusi: nuovoTurno.giorni_esclusi || [], inattivita: safeArray(nuovoTurno.inattivita) }];
      });
  ...
  const handleEdit = (row: Veicolo) => {
    ...
    if (dispo) {
      ...
      setInattivita(safeArray(dispo.inattivita));
    } else {
      setEditingId(null);
      setStart('');
      setFine('');
    ...
    () =>
      veicoli.map(v => {
        ...
        return {
          ...
          inattivita: safeArray(dispo?.inattivita),
        };
      }),
