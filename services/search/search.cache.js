...
export const CacheStore = {
  ...
  prenotazioniCache: new Map() // NUOVO: Mappa corsa_id -> Array di prenotazioni
};
...
export const upsertCorsa = (c) => {
  ...
  const newCorsa = {
    ...
    distanza: Number(c.distanza || oldData?.distanza || 0), decodedCoords, bbox, path_geohashes: geohashes,
...
export async function loadCachesUltra(force = false) {
  ...
  try {
    ...
    // 2. Carica Prenotazioni attive per disponibilità segmentata
    ...
    console.log(`📦 [CACHE] Pronta. Corse: ${CacheStore.corseCache.size}, Prenotazioni indicizzate per ${CacheStore.prenotazioniCache.size} corse.`);
  } finally {
    client.release();
  }
}