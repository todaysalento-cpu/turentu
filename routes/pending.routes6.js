  ...
  try {
    ...
    const result = await client.query(
      `SELECT p.*, u.nome AS cliente_nome
       ...
       WHERE p.veicolo_id = $1 AND p.stato <> 'accettata'
       ORDER BY p.id ASC`,
      [veicoloId]
    );
...
router.post('/:id/accetta', async (req, res) => {
  ...
  try {
    ...
    const returningFields = `
      *, ST_X(origine::geometry) AS origine_lon, ST_Y(origine::geometry) AS origine_lat,
    ...
    if (allSlots && pendingRes.rows[0].request_id) {
      const result = await client.query(
        `UPDATE pending SET stato = 'accettata' WHERE request_id = $1 RETURNING ${returningFields}`,
    ...
    for (const p of acceptedPendings) {
      ...
      // Recupero nome autista
      const driverRes = await client.query(
        `SELECT u.nome AS driver_nome FROM veicolo v
      ...
      // --- 🔔 Notifica al cliente con prezzo e nome autista ---
