...
export default function PendingList({ pending = [], loadingId, onAccetta }: PendingListProps) {
  ...
  const formatDateTime = (dt?: string) => {
    ...
    if (dateLocal.toDateString() === now.toDateString())
      return `Oggi alle ${dateLocal.toLocaleTimeString('it-IT', optionsTime)}`;
    if (dateLocal.toDateString() === tomorrow.toDateString())
      return `Domani alle ${dateLocal.toLocaleTimeString('it-IT', optionsTime)}`;

    return dateLocal.toLocaleString('it-IT', optionsFull);
  };

  // ---------------- Indirizzo ----------------
  const displayAddress = (addr?: string) => (addr ? addr.split(',').slice(0, 2).join(', ') : 'N/D');
