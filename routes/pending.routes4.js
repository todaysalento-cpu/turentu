...
router.post('/:id/accetta', async (req, res) => {
  ...
  try {
    ...
    // 🔹 Ricostruisci coordinate
    acceptedPendings = acceptedPendings.map(p => ({
      ...p,
      origine: p.origine_lat != null && p.origine_lon != null
        ? { lat: Number(p.origine_lat), lon: Number(p.origine_lon) }
        : null,
      destinazione: p.destinazione_lat != null && p.destinazione_lon != null
        ? { lat: Number(p.destinazione_lat), lon: Number(p.destinazione_lon) }
        : null,
