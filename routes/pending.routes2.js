...
router.post('/', async (req, res) => {
  try {
    const {
      ...
      request_id,
      corsa_id,
      expires_at, // <- già destructurato
    } = req.body;

    // Inserimento in DB con expires_at incluso
    const result = await pool.query(
      `INSERT INTO pending 
        (veicolo_id, cliente_id, start_datetime, durata, posti_richiesti, tipo_corsa, prezzo, origine, destinazione, stato, payment_intent_id, request_id, corsa_id, expires_at)
