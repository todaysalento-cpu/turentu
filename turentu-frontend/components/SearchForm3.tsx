...
export default function SearchForm({ form, dispatch, onSubmit, loading, isMobile, showResults = false }: SearchFormProps) {
  ...
  const [locationKey, setLocationKey] = useState(0); // forza remount LocationSheet
  const panelRef = useRef<HTMLDivElement>(null); // ref per click fuori
  ...
  const DesktopField = ({ label, value, icon, onClick, isActive, placeholder }: any) => (
    <motion.div
      ...
      className={`
        ...
        border-2 ${isActive ? "border-[#ff3036]" : "border-black/80"}
        min-w-[160px] max-w-[200px] transition-all duration-200
      `}
    >
      ...
      <span className="text-sm font-medium text-slate-800 truncate">{value || placeholder}</span>
    </motion.div>
  );
  ...
  const openField = (field: ActiveInput, queryVal: string) => {
    ...
    setLocationKey(prev => prev + 1); // forza remount LocationSheet
  };
  ...
  const MobileForm = (
    <div className="w-full max-w-[1280px] mx-auto flex flex-col gap-4 p-4 relative">
      ...
      <AnimatePresence>
        {activeInput && isMobile && (
          <motion.div
            ref={panelRef} // 🔹 ref qui
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
            transition={{ type: "spring", damping: 25 }}
            className="fixed bottom-0 left-0 w-full rounded-t-2xl z-50 p-6 h-[60vh] flex flex-col bg-white"
          >
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <AnimatePresence>
          {activeInput && (activeInput === "origine" || activeInput === "destinazione" || activeInput === "posti") && (
            <motion.div
              ref={panelRef} // 🔹 ref qui anche in desktop
              initial={{ opacity: 0, y: -6 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -6 }}
              className="absolute top-full mt-2 w-[420px] rounded-xl border border-slate-200 p-4 z-50 bg-white"
            >
              {["origine", "destinazione"].includes(activeInput) && (
                <LocationSheet
                  key={locationKey}
