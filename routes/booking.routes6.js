...
router.post('/payment-intent', authMiddleware, async (req, res) => {
  try {
    ...
    for (const slot of slots) {
      ...
      if (typeof durata === 'object') durataMin = durata.minutes ?? durata.seconds ?? durataMin;
      if (isNaN(durataMin) || durataMin <= 0) throw new Error("Durata non valida");

      const expiresAt = new Date(Date.now() + 30 * 60 * 1000);

      // 🔹 Inserimento nel DB con coordinate e indirizzi forniti
      ...
      const formatted = {
        ...
        // ✅ Manteniamo solo i campi del DB
        // origineAddress e destinazioneAddress rimossi
      };
      pendingRows.push(formatted);

      // 🔹 Socket aggiornamento driver
      ...
      if (driverId) {
        io.to(`autista_${driverId}`).emit('pending_update', {
          ...
          origine: formatted.origine,
          destinazione: formatted.destinazione,
          origine_address: localitaOrigine,
          destinazione_address: localitaDestinazione,
          prezzo: formatted.prezzo,
          stato: formatted.stato,
        });
      }
    }
