...
interface CorseGiornataProps {
  ...
  onSelectCorsa?: (corsa: any | null) => void; // callback per aprire/chiudere chat
}
...
export default function CorseGiornata({ corse = [], onToggleCorsa, loadingId, onSelectCorsa }: CorseGiornataProps) {
  ...
  useEffect(() => {
    console.log('CorseGiornata effect corse:', corse);
    setCorseGiornata(
      (corse || []).filter(c => {
            ...
            >
              ...
              <div className="flex gap-1 justify-end">
                {showIniziaBtn && (
                  <button
                    onClick={() => {
                      console.log('Click Inizia corsa:', c);
                      onToggleCorsa?.(c);
                      setExpandedMap(c.id);
                      const corsaForChat = {
                        ...c,
                      ...
                      console.log('onSelectCorsa payload:', corsaForChat);
                ...
                {onToggleCorsa && stato === 'in_corso' && (
                  <>
                    ...
                    <button
                      onClick={() => {
                        console.log('Click Termina corsa:', c);
                        onToggleCorsa?.(c);
                        setExpandedMap(null);
                        onSelectCorsa?.(null);
                      }}
