...
// ----------------------- REFRESH NOTIFICATIONS AL LOGIN -----------------------
function NotificationRefresher() {
  const { user } = useUser();
  const { refreshNotifications } = useNotifications();

  // Ricarica le notifiche subito quando l'utente effettua il login
  useEffect(() => {
    if (user?.id) {
  ...
  return (
    <html lang="it">
      <body className="bg-gray-100">
        {/* 1️⃣ UserProvider sempre primo */}
        <UserProvider>
          {/* 2️⃣ SocketProvider per connessione globale */}
          <SocketProvider>
            {/* 3️⃣ NotificationProvider accede a user e socket */}
            <NotificationProvider>
              {/* 🔹 Carica Google Maps API */}
              <GoogleMapsLoader />

              {/* 🔹 Aggiorna notifiche al login */}
              ...
              <div className="min-h-screen flex flex-col">
                ...
                <main className="flex-1 mt-[48px] w-full">
