...
import { sendPush } from '../services/notifications/push.service.js'; // 📲 PUSH SERVICE
...
function generatePendingMessage({ role, startAddress, endAddress, startDatetime }) {
  ...
  if (role === 'autista')
    return `Nuova Richiesta - ${startAddress} → ${endAddress}${timeStr ? ' - ' + timeStr : ''}`;
...
router.post('/payment-intent', authMiddleware, async (req, res) => {
  try {
    ...
    for (const slot of slots) {
      ...
      if (driverId) {
        const notifDriver = await pool.query(
        ...
        const notification = notifDriver.rows[0];

        // 🔔 SOCKET
        sendNotification({ userId: driverId, role: 'autista', notification });

        // 📲 PUSH
        const driverTokens = await pool.query(
          `SELECT push_token FROM utente_push_tokens WHERE user_id=$1`,
          [driverId]
        );

        for (const t of driverTokens.rows) {
          await sendPush(
            t.push_token,
            'Nuova richiesta 🚖',
            messageDriver,
            { type: 'pending', requestId }
          );
        }
      }

      // ===================== CLIENTE =====================
      const notifCliente = await pool.query(
        `INSERT INTO notifications(user_id, type, message, seen, created_at)
         VALUES ($1, 'pending', $2, false, NOW()) RETURNING *`,
        [clienteId, messageCliente]
      );

      const notificationC = notifCliente.rows[0];

      // 🔔 SOCKET
      sendNotification({ userId: clienteId, role: 'cliente', notification: notificationC });

      // 📲 PUSH
      const clienteTokens = await pool.query(
        `SELECT push_token FROM utente_push_tokens WHERE user_id=$1`,
        [clienteId]
      );

      for (const t of clienteTokens.rows) {
        await sendPush(
          t.push_token,
          'Prenotazione creata 🏁',
          messageCliente,
