...
import { getAddress } from '../utils/geo.util.js'; // 🔹 Import geocoding
...
router.post('/payment-intent', authMiddleware, async (req, res) => {
  try {
    ...
    for (const slot of slots) {
      ...
      // 🔹 Ottieni indirizzi tramite geocoding lato server
      const origineAddress = await getAddress(origineCoord);
      const destinazioneAddress = await getAddress(destinazioneCoord);
    ...
    res.json({
      ...
      pending: pendingRows.map(p => ({
        ...
        origineAddress: p.origineAddress,
        destinazioneAddress: p.destinazioneAddress,
      })),
      requestId,
    });
