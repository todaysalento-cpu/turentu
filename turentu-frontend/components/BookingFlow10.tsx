      ...
      case 0:
        return (
          <button
            ...
            onClick={() => setStep(1)}
            disabled={selectedIds.length === 0} // non passa se non selezionato
          >
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.div
            className="fixed inset-x-0 bottom-0 z-50 w-full max-h-[90vh] flex flex-col"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
            transition={{ type: "spring", damping: 25 }}
          >
            <BookingCard header={renderHeader()} footer={renderFooter()} className="flex-1 overflow-hidden">
              <div className="flex flex-col h-full">
                {/* STEP 0: selezione slot */}
                {step === 0 && (
                  <RisultatiSelection
                ...
                {/* STEP 1: dettagli della prenotazione */}
                {step === 1 && (
                  <StepDettagli
                    type={type}
