...
export async function cercaSlotUltra(richiesta) {
  ...
  const slotResults = await Promise.all(topSlots.map(async slot => {
    ...
    // Distanza da usare per il prezzo = distanza tra origine e destinazione
    const kmPrezzo = haversineDistance(
      richiesta.coord,
      richiesta.coordDest
    );
    ...
    return {
      ...
      corseCompatibili: [],
      slot_libero: true,
      distanzaKm: slot._distanzaKm, // distanza per ordinamento/visualizzazione
      prezzo,
      euro_km: Number(v.euro_km ?? 1.0),
      stato: 'libero'
      posti_richiesti: richiesta.posti_richiesti
    };
  }));
  ...
  const corsaResults = await Promise.all(topCorse.map(async corsa => {
    ...
    // Distanza da usare per il prezzo = distanza memorizzata nel DB
    const kmPrezzo = Number(corsa.distanza ?? 0);
    ...
    return {
      ...
      posti_richiesti_richiesta.posti_richiesti
    };
  }));
