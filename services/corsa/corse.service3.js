...
export async function getCorseByAutista(driver_id, status = '') {
  ...
  return res.rows.map(c => ({
    ...
    origine:
      c.origine_lat != null && c.origine_lon != null
        ? { lat: c.origine_lat, lon: c.origine_lon }
        : null,
    destinazione:
      c.destinazione_lat != null && c.destinazione_lon != null
        ? { lat: c.destinazione_lat, lon: c.destinazione_lon }
        : null,
    ora: c.start_datetime
      ? new Date(c.start_datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      : 'N/D',
    data: c.start_datetime
      ? new Date(c.start_datetime).toLocaleDateString([], { day: '2-digit', month: '2-digit', year: 'numeric' })
      : 'N/D',
  }));
}
