...
export async function cercaSlotUltra(richiesta) {
  ...
  const slotResults = await Promise.all(topSlots.map(async slot => {
    ...
    return {
      ...
      stato: 'libero',
      posti_richiesti: richiesta.posti_richiesti // ✅ virgola aggiunta
    };
  }));
  ...
  const corsaResults = await Promise.all(topCorse.map(async corsa => {
    ...
    const veicolo = veicoliCache[corsa.veicolo_id];
    ...
    return {
      ...
      modello: veicolo?.modello ?? 'N/D', // ✅ prende modello dal veicolo
      servizi: veicolo?.servizi ?? [],   // ✅ prende servizi dal veicolo
      coord,
      corseCompatibili: [corsa],
      slot_libero: false,
      distanzaKm: 0,
      prezzo,
      euro_km: Number(veicolo?.euro_km ?? 1.0),
      stato: corsa.stato ?? 'condiviso',
      posti_richiesti: richiesta.posti_richiesti // ✅ corretta
    };
  }));
